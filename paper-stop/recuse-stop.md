...
|---|---|---|---|
| Halt, in-band (n=20) | 0/20 (0%) | 0/20 (0%) | ~2.5 |
...
**F2 — channel salience dominates *noticing*.** In-band halt acknowledged 0/20; the same halt in the prompt channel acknowledged 20/20. A signal buried in tool output is skimmed past: the agent reads command output for the data it wants and treats the appended sentinel as noise. Channel choice has a large, clean effect — but on noticing, not on stopping.
